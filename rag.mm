...
where 
  - eqs, positive and nonzero are lists of polynomials
  - opts is a set indicating some options

Warning: at the moment, eqs must be empty (otherwise an error "Not
implemented yet" is raised.
...
SemiAlgebraicSolve requires the msolve library to be installed with
its interface to Maple.
...
Possible options are: 
  - "isempty"=INT (default value is 0). 
    ...
    In case, it is not empty, it will return a witness point. 
